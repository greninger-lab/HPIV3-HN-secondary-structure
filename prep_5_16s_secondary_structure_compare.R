library(ggplot2)
setwd('/Users/gerbix/Documents/garrett/prep_5_results')

#reading amplicon profile for depth info
amplicon_profile_table<-read.table('/Users/gerbix/Documents/garrett/prep_5_results/prep_5_ecoli/prep5_ecoli_16s_amplicon_profile.txt', header=TRUE)

#read reference map file
reference_16s_map<-read.csv('/Users/gerbix/Documents/garrett/prep_5_results/shapemapper_out/227_ecoli_16s_unreversed/16S.1m7.map', sep = '\t', col.names = c('nucleotide_position', 'normalized_reactivity', 'stderr', 'sequence'))
...
garret_16s_map<-read.csv('/Users/gerbix/Documents/garrett/prep_5_results/prep_5_ecoli/prep5_ecoli_16s_amplicon.map', sep = '\t',col.names = c('nucleotide_position', 'normalized_reactivity', 'stderr', 'sequence'))
...
absolute_value_plot<-ggplot(Absolute_value_dataframe, aes(x=Absolute_value_dataframe$removed_garret_16s_map.nucleotide_position, y= Absolute_value_dataframe$absDeltareactivity)) + 
  ...
  theme_classic() + 
  ggtitle('delta normalized reactivity by position') + 
...
absolute_value_plot
ggsave(plot = absolute_value_plot, "difference_in_normalized_reactivity_by_nucleotide_position.pdf")
...
reactivity_by_modified_depth<-ggplot(combined_depth_table, aes(x=combined_depth_table$Modified_mapped_depth, y= combined_depth_table$delta_reactivity)) + 
  ...
  ylab('Garret Reactivity - reference reactivity') + 
  theme_classic() + 
  ggtitle('difference in reactivity by modified sample depth') +
  geom_hline(yintercept = 0) + 
  geom_point()+ 
  geom_smooth(method = "loess") 
...
reactivity_by_untreated_depth<-ggplot(combined_depth_table, aes(x=combined_depth_table$Untreated_mapped_depth, y= combined_depth_table$delta_reactivity)) + 
  ...
  ylab('Garret Reactivity - reference reactivity') + 
  theme_classic() + 
  ggtitle('difference in reactivity by untreated sample depth') + 
  geom_hline(yintercept = 0) + 
  geom_point()+ 
  geom_smooth(method = "loess") 
...
reactivity_by_denatured_control_depth<-ggplot(combined_depth_table, aes(x=combined_depth_table$Denatured_mapped_depth, y= combined_depth_table$delta_reactivity)) + 
  ...
  ylab('Garret Reactivity - reference reactivity') + 
  theme_classic() + 
  ggtitle('difference in reactivity by denatured control sample depth') + 
  geom_hline(yintercept = 0) + 
  geom_point() +
  geom_smooth(method = "loess") 
...
write.csv(combined_depth_table, 'depth_information_table.csv', sep =',')
